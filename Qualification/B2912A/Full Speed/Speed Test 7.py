...
for channel in channels:
	...
	inst.write(':sense{}:function \"curr\"'.format(channel))
	# inst.write(':sense{}:curr:nplc 1'.format(channel))
	inst.write(':sense{}:curr:prot 10e-6'.format(channel))
	
	inst.write(':sense{}:curr:range:auto on'.format(channel))
	# inst.write(':sense{}:curr:range 1E-8'.format(channel))
	# inst.write(':sense{}:curr:range:auto:mode speed'.format(channel))
	# inst.write(':sense{}:curr:range:auto:THR 80'.format(channel))
	inst.write(':sense{}:curr:range:auto:LLIM 1E-8'.format(channel))
	
	inst.write(":source1:voltage:mode sweep")
	inst.write(":source2:voltage:mode sweep")

	inst.write(":source1:voltage:start {}".format(Vds))
	inst.write(":source1:voltage:stop {}".format(Vds)) 
	inst.write(":source1:voltage:points {}".format(Npoints))
...
# for vgs in np.linspace(0,Vgs,50):
# 	inst.write(':source2:voltage {}'.format(vgs))
# 	time.sleep(0.1)
# 	inst.query_binary_values(':measure? (@1:2)')
...
# for vgs in np.linspace(Vgs,0,50):
# 	inst.write(':source2:voltage {}'.format(vgs))
# 	time.sleep(0.1)
# 	inst.query_binary_values(':measure? (@1:2)')
